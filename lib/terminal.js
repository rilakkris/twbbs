...
class Terminal extends Writable {
  ...
  constructor(options = {}) {
    ...
    for(let i = 0; i < this.height; i++)
      this.text[i] = Buffer.alloc(this.width + 1);

    this.color = new Array(this.height + 1);
    for(let i = 0; i < this.height; i++)
      this.color[i] = Buffer.alloc(this.width + 1);
  ...
  inspect() {
    return this.text
      .map((line) => {return iconv.decode(line,'Big5');})
      .join('\n')
  }
  ...
  _consumeToken(token) {
    ...
    const tokenHandler = {
      ...
      'SGR': token => {
        // TODO
        this.color[this.cursor[0]][this.cursor[1]] = token.color;
      },
      'CUP': token => {
        if (token.position === undefined) {
          this.cursor = [1, 1];
        }
        else this.cursor = [token.row, token.column];
      },
      'DAT': token => {
        //console.log(iconv.decode(token.raw, 'Big5'));
        for(let i = 0; i < token.raw.length; i++) {
          this.text[this.cursor[0]][this.cursor[1]] = token.raw[i];
          if (this.cursor[1]++ === this.width) {
            this.cursor[0]++;
            this.cursor[1] = 1;
          }
        }
